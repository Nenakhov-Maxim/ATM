...
class TaskTransferConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        if data['message'] == "start":
            print('Начали собирать информацию о заданиях на линии: ', self.area_id['line_name'])
            print('==============================================================')         
    ...
    def build_task_payload(self, task):
        return {
            ...
            'task_order_number': task.order_label,
            'task_profile_material': task.task_profile_material,
            'planned_shift_label': task.planned_shift_label,
            'coating_revision': task.coating_revision,
            'is_stock': bool(task.stock_source_id),
